...
const Suppliers = () => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await supplierAPI.index({ search, page, per_page: 10 });
    ...
    const openEdit = (s) => { setEditing(s); setForm({ name: s.name, phone: s.phone || '', email: s.email || '', address: s.address || '' }); setShowModal(true); };

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (editing) { await supplierAPI.update(editing.id, form); Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Supplier berhasil diupdate', timer: 1500, showConfirmButton: false }); }
            else { await supplierAPI.store(form); Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Supplier berhasil ditambahkan', timer: 1500, showConfirmButton: false }); }
    ...
    const handleDelete = (supplier) => {
        Swal.fire({
            title: 'Hapus Supplier?', text: `Yakin ingin menghapus ${supplier.name}?`, icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#FF6B00', cancelButtonColor: '#6B7280',
            confirmButtonText: 'Ya, Hapus', cancelButtonText: 'Batal',
        }).then(async (result) => {
            if (result.isConfirmed) {
                try { await supplierAPI.destroy(supplier.id); Swal.fire({ icon: 'success', title: 'Berhasil', timer: 1500, showConfirmButton: false }); fetchData(); }
    ...
    const columns = [
        ...
        { header: 'Telepon', accessor: 'phone', render: (row) => <span className="text-gray-500">{row.phone || '-'}</span> },
        { header: 'Email', accessor: 'email', render: (row) => <span className="text-gray-500">{row.email || '-'}</span> },
        { header: 'Alamat', accessor: 'address', render: (row) => <span className="text-gray-500 max-w-xs truncate block">{row.address || '-'}</span> },
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button onClick={openCreate} className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm font-medium">+ Tambah Supplier</button>
            </div>
            <DataTable columns={columns} data={suppliers} loading={loading} search={search} onSearchChange={setSearch} meta={meta} onPageChange={setPage} />

            {showModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-gray-100"><h3 className="text-lg font-semibold">{editing ? 'Edit Supplier' : 'Tambah Supplier'}</h3></div>
